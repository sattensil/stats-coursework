...
***************************************; 
* Get the same results using previuos methods; 
***************************************; 

PROC SQL; 
 CREATE TABLE INTERCEPT_EXCEPT AS
  SELECT A.*
   FROM  USE_FOR_ALL  A
   where a.id notin 
  (select b.id from ICU_SUB_SET3 B)
   order by a.id
   ;
   quit; 
   ...
   proc print data=intercept_except; 
    title 'intercept except'; 

	run; 


 ***************************************************; 
 * Lets dedup the icu_sub_set3 and see the difference with having and having all; 
 ****************************************************; 

 proc contents data=icu_sub_set3; 

 run; 

 proc sql; 
 create table icu_sub_set4 as 
  select distinct  
  a.AGE,
a.BIC,
a.CAN,
...
a.TYP
  ...
  proc sql;
create table except_no_all4 as 
 select * 
  from use_for_all 
  except
    select *  
...
quit;
 ...
 proc print data=except_no_all4; 
 title 'except_no_all4'; 

 run; 


    proc sql;
create table except_all4 as 
 select * 
  from use_for_all 
  except all
    select *  
